...
public class MapdbCacheProvider implements CacheProvider, InitializingBean, DisposableBean {
    ...
    @Override
    public <K, V> Map<K, V> newGenericMapCache(String cacheName, Class<K> keyClazz, Class<V> valueClazz) {
        MVMap<byte[], byte[]> mvMap = mvStore.openMap(cacheName);
        return new GenericMapCache<>(mvMap, keyClazz, valueClazz);
    }

    @Override
    public <K, V> Map<K, V> newGenericMapCache(String cacheName, TypeReference<K, V> typeReference) {
        MVMap<byte[], byte[]> mvMap = mvStore.openMap(cacheName);
