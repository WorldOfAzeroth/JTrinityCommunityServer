...
public class Db2FileReader<T> {
    ...
    private final Object idField;
    private final Object idFieldIndex;

    public static <T> void read(String file, Class<T> clazz, Consumer<T> consumer) {
        Path db2Path1 = Paths.get(file);
        Db2FileReader<T> instance = new Db2FileReader<>(Paths.get(file), consumer);
        instance.loadSchema(clazz);
    }

    private final Path db2Path;
    private final Map<Integer, T> rows;
    private final Map<Integer, Integer> copyTable;
    private final Map stringTable;
     ...
     */
    private Db2FileReader(Path db2Path, Consumer<T> consumer) {
     ...
     */
    T getRow(int recordID, Class<T> clazz) {
        ...
        if (!this.isLoaded)
            throw new IllegalStateException("Attempted to read a data table row before table was loaded.");

        // Look this row up as a normal entry.
		T record = this.rows.get(recordID);
        if (record != null)
            return record;

        // Check if the copy table contains a mapping entry.
		Integer copyID = this.copyTable.get(recordID);
        if (copyID != null) {
			T copy = this.rows.get(copyID);
            if (copy != null) {
                T tempCopy = copy.clone();
                tempCopy.setId(recordID);
                return tempCopy;
            }
        }
    ...
    List<T> getAllRows() {
        ...
        if (!this.isLoaded)
            throw new IllegalStateException("Attempted to read a data table rows before table was loaded.");
     ...
     */
    private void loadSchema(Class<T> clazz) {
     ...
     */
    async parse(data) {
        log.write('Loading DB file %s from CASC', this.db2Path);

        if (!data)
            data = await core.view.casc.getFileByName(this.db2Path, true, false, true);
        ...
        log.write('Processing DB file %s as %s', this.db2Path, format.name);
        ...
        for (let sectionIndex = 0; sectionIndex < sectionCount; sectionIndex++) {
            ...
            if (header.tactKeyHash !== BigInt(0)) {
                ...
                if (isZeroed) {
                    log.write("Skipping all-zero encrypted section " + sectionIndex + " in file " + this.db2Path);
                    continue;
                }
            }
        ...
        log.write('Parsed %s with %d rows', this.db2Path, this.size);
        this.isLoaded = true;
    }
}
